...
class _AnalyticsPageState extends State<AnalyticsPage> {
  ...
  Future<void> _generatePdf(BuildContext context, List<OperationRecord> sessions) async {
          ...
          build: (pw.Context c) => pw.Padding(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                _pdfEnvStats(op, t),
                pw.SizedBox(height: 16),
                pw.Text(t.analytics_notes, style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
                pw.Text(t.analytics_notesBody, style: const pw.TextStyle(fontSize: 10)),
              ],
            ),
          ),
        ),
    ...
    await Printing.sharePdf(
      ...
      filename: 'NiceRice_${DateTime.now().toIso8601String().replaceAll(":", "-")}.pdf',
    );
  }
  ...
  pw.Widget _pdfSummary(OperationRecord op, AppLocalizations t) {
    ...
    final intended = intendedUseFromTarget(targetMc);
    ...
    return pw.Table(
      ...
      children: [
        ...
        _row(t.analytics_ended, op.endedAt == null ? '—' : DateFormat('MMM d, HH:mm:ss').format(op.endedAt!)),
  ...
  pw.TableRow _row(String k, String v) {
    return pw.TableRow(
      children: [
        pw.Padding(padding: const pw.EdgeInsets.symmetric(vertical: 6), child: pw.Text(k, style: const pw.TextStyle(fontSize: 11))),
        pw.Padding(
  ...
  pw.Widget _pdfEnvStats(OperationRecord op, AppLocalizations t) {
    ...
    if (rows.isEmpty) rows.add(_row(t.analytics_environment, t.analytics_noEnvData));
...
class _SectionCard extends StatelessWidget {
  ...
  const _SectionCard({required this.title, required this.child, required this.titleStyle});
  @override
  Widget build(BuildContext context) {
    return Card(
      child: Padding(
...
class _EmptyState extends StatelessWidget {
  ...
  final double? height;
  const _EmptyState({required this.message, required this.textStyle, this.height});
  @override
  Widget build(BuildContext context) {
  ...
  Widget _btn(BuildContext context, String label, _FilterRange me) {
    ...
    return Expanded(
      child: OutlinedButton(
        ...
        child: Text(label, textAlign: TextAlign.center, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w700)),
      ),
    );
  }
...
// -----------------------------------------------------------------------------
// Environment chart (kept from your original, lightly trimmed)
// -----------------------------------------------------------------------------
class _EnvChart extends StatelessWidget {
  ...
  final double scale;
  final Widget? empty;
  const _EnvChart({required this.temps, required this.rhs, this.height, this.scale = 1.0, this.empty});
        ...
        LineChart(
          LineChartData(
            ...
            titlesData: FlTitlesData(
              ...
              bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 28, interval: (xMax - xMin) / 4.0, getTitlesWidget: (v, m) => Padding(padding: const EdgeInsets.only(top: 8), child: Text(xLabel(v), style: tS(11))))),
              leftTitles: AxisTitles(
                axisNameWidget: Padding(padding: const EdgeInsets.only(bottom: 4), child: Text('°C', style: tS(10))),
                sideTitles: SideTitles(showTitles: true, reservedSize: 36, interval: ((tMax - tMin) / 5).clamp(1, 100).toDouble(), getTitlesWidget: (v, m) => Text(v.toStringAsFixed(0), style: tS(11))),
              ),
            ),
            ...
            LineChartData(
              ...
              titlesData: FlTitlesData(
                ...
                rightTitles: AxisTitles(
                  ...
                  sideTitles: SideTitles(showTitles: true, reservedSize: 36, interval: ((hMax - hMin) / 5).clamp(1, 100).toDouble(), getTitlesWidget: (v, m) => Text(v.toStringAsFixed(0), style: tS(11))),
                ),
              ),
      ...
      child: LineChart(
        LineChartData(
          ...
          titlesData: FlTitlesData(
            topTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
            rightTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
            bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 28, interval: max(60, spots.last.x / 4), getTitlesWidget: (v, m) => Padding(padding: const EdgeInsets.only(top: 8), child: Text(xLabel(v), style: GoogleFonts.poppins(fontSize: 11, color: cs.onSurface.withOpacity(0.7)))))),
            leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 40, getTitlesWidget: (v, m) => Text('${v.toStringAsFixed(0)}%', style: GoogleFonts.poppins(fontSize: 11, color: cs.onSurface.withOpacity(0.7))))),
          ),
...
class _EstimatedMcInterpretationCard extends StatelessWidget {
  ...
  const _EstimatedMcInterpretationCard({required this.op, required this.titleSize, this.bulletGap = 4});
  ...
  Widget build(BuildContext context) {
    ...
    return _SectionCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8),
        ...bullet('Start MC', '${initialMc.toStringAsFixed(1)}%'),
        ...bullet('Target MC', '${targetMc.toStringAsFixed(1)}%'),
        ...bullet('Moisture loss', '${loss.toStringAsFixed(1)}%'),
        const SizedBox(height: 6),
        Text('Summary', style: ts(13, w: FontWeight.w800, c: context.brand)),
        const SizedBox(height: 4),
        Text('• Moisture content decreased from ${initialMc.toStringAsFixed(1)}% to ${predictedFinal.toStringAsFixed(1)}% within ${fmtHMS(dur)}.', style: ts(13, w: FontWeight.w600)),
        Text('• The moisture loss in this session is ${loss.toStringAsFixed(1)}%.', style: ts(13, w: FontWeight.w600)),
      ]),
    );
  }
}
...
// -----------------------------------------------------------------------------
// Session summary (uses target→intended mapping)
// -----------------------------------------------------------------------------
